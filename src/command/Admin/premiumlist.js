...
module.exports = {
  ...
  run: async (client, message, args) => {
    if (message.author.roles.cache.find((x) => x.name === "🔑")) {
      ...
      await Promise.all(
        keysSorted.map(async (x, i) => {
          ...
          const getManagerData = await client.db.get(`premManager.${x}`);
          const getMember = await client.users.fetch(x);
          const getManager = await client.users.fetch(getManagerData);

          arr.push(
            `\`[${i + 1}]\` - **${getMember.tag}** - <t:${Math.ceil(
              getUserData / 1000
            )}:R> -> \`Managed by:\` *${getManager ? getManager : "Unknown"}*`
          );
        })
      );
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          ["", "", arr.slice(0, getData.length).join("\n")].join("\n")
        )
        .setColor("#9BEEFF")
        .setFooter({
