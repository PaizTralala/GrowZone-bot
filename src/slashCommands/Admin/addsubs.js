		...
		if (
			...
			if (member && duration) {
				...
				if (exist) {
					...
					await client.db.set(`premManager.${member.user.id}`, interaction.user.id);

					const getLatestUserDate = await client.db.get(`premiumUser.${member.user.id}`);
					const formattedDate = Math.ceil(getLatestUserDate / 1000);

					interaction.reply({
						embeds: [
							embed
								.setAuthor({ name: 'Add Subscription Status: SUCCESS', iconURL: member.user.displayAvatarURL({ dynamic: true }) })
								.setDescription(`Succesfully added \`${duration} Days\` more of premium subscription to **${member.user.tag}**`)
								.addFields([
									{
					...
					member.send({
						embeds: [
							extendedPremiumEmbed
								.setAuthor({ name: 'Premium Status: EXTENDED', iconURL: member.user.displayAvatarURL({ dynamic: true }) })
					...
					const getNewSubDate = await client.db.get(`premiumUser.${member.user.id}`);
					...
					interaction.reply({
						embeds: [
							embed
								.setAuthor({ name: 'Add Subscription Status: SUCCESS', iconURL: member.user.displayAvatarURL({ dynamic: true }) })
								.setDescription(`Succesfully added \`${duration} Days\` of premium subscription to **${member.user.tag}**`)
								.addFields([
									{
										name: `Premium Manager`,
										value: `> *${interaction.user.tag}*`
					...
					try {
						member.send({
							embeds: [
								extendedPremiumEmbed
									.setAuthor({ name: 'Premium Status: ACTIVE', iconURL: member.user.displayAvatarURL({ dynamic: true }) })
