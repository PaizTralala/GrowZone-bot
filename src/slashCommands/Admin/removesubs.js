...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (interaction.member.roles.cache.find((x) => x.name === 'Admin')) {
      if (member) {
        ...
        if (exist) {
          ...
          interaction.reply({
            embeds: [
              embed
                .setAuthor({
                  name: 'Delete Subscription Status: SUCCESS',
                  iconURL: member.user.displayAvatarURL({ dynamic: true }),
                })
                .setDescription(
                  `Succesfully deleted **${member.user.tag}** from premium user`
                ),
            ],
          });
          ...
          member.send({
            embeds: [
              embed
                .setAuthor({
                  name: 'Premium Status: REMOVED (FORCED)',
                  iconURL: member.user.displayAvatarURL({ dynamic: true }),
                })
                .setDescription(
          ...
          interaction.reply({
            embeds: [
              embed
                .setAuthor({
                  name: 'Delete Subscription Status: FAILED',
                  iconURL: member.user.displayAvatarURL({ dynamic: true }),
                })
                .setDescription(
                  `**${member.user.tag}** Doesn't have any active subscription!`
                ),
            ],
          });
