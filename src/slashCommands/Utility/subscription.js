  ...
  run: async (client, interaction) => {
    ...
    if (!premiumLeft) {
      return interaction.reply({
        embeds: [
          embed
            .setAuthor({
              name: 'Subscription Status: FAILED',
              iconURL: member.user.displayAvatarURL({ dynamic: true }),
            })
            .setDescription(
              `User with tag **${member.user.tag}** doesn't have any active subscription!`
            ),
        ],
      });
    }
    ...
    interaction.reply({
      embeds: [
        embed
          .setAuthor({
            name: `Subscription information for: ${member.user.tag}`,
            iconURL: member.user.displayAvatarURL({ dynamic: true }),
          })
          .setDescription(
            `Subscription ends <t:${premiumEndsInFormatted}:R> - <t:${premiumEndsInFormatted}>`
          ),
      ],
    });
  },
